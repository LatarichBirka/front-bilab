...
const ProcedureDetail = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://localhost:7235/api/Procedure/'+ id)
      .then(response => response.json())
      .then(data => setProcedure(data.value))
      ...
      .then(response => {
        ...
        .then(data => {
          ...
          if (data.value.length === 0) {
            setTimes([{ fromTime: "У работника нет свободного времени на этот день.", toTime: "" }]);
          } else {
            setTimes(data.value);
          }
        })
  ...
  const handleEmployeeChange = (employee) => {
    ...
    // console.log(employee);
    if (date) {
      // const dayOfWeek = date.day(); // Получаем день недели как число
      const accessToken = localStorage.getItem('accessToken');
    ...
    <div>
      ...
      <div className="procedure-detail-container">
        <div className="procedure-detail-content">
          <div className="procedure-detail-text">
